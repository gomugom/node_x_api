...
describe('POST /join', () => {
    ...
    beforeEach((done) => {
        // 로그인
        agent.post('/auth/login').send({ // send를 통해서 body 파라미터 전달
            email: 'dudfhd705@gmail.com',
            password: '!2345Qwert',
        }).end(done);
    });
    test('로그인 되어있으면 회원가입 진행이 안되어야함', (done) => {
...
describe('POST /login', () => {
   test('로그인 수행', (done) => {
       ...
       request(app).post('/auth/login').send({ // send를 통해서 body 파라미터 전달
           email: 'dudfhd705@gmail.com',
           password: '!2345Qwert',
       }).expect('Location', '/') // redirect('/') 의 의미 => expect('Location', '/').expect(302)
         .expect(302, done); // 끝났음을 알려줘야 jest가 한없이 기다리지 않음
   });

    test('회원가입한적 없는 사용자 로그인 수행', (done) => {
        // 이렇게하면 app에 post로 /auth/login을 호출하는 샘이 됨
        request(app).post('/auth/login').send({ // send를 통해서 body 파라미터 전달
